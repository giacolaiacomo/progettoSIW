...
<f:view>
  ...
  <h:form>
    ...
    <h1>My Basket</h1>
    <c:choose>
      <c:when test="${orderController.order.closed==true}">
        <h4>The order has been closed and is being processed, please create another order</h4>
        <h:commandButton styleClass="btn btn-default" value="New Order" action="#{orderController.createOrder}">
          <f:setPropertyActionListener value="#{customerController.customer}" target="#{orderController.customer}"/>
        </h:commandButton>
      </c:when>
      <c:when test="${orderController.order.closed!=true && orderController.order!=null}">
